...
class FreetypeHandle {
    ...
    friend FontHandle * loadFont(FreetypeHandle *library, const void *memory, size_t memorySize);
...
FontHandle * loadFont(FreetypeHandle *library, const void *memory, size_t memorySize) {
    if (!library)
        return NULL;
    FontHandle *handle = new FontHandle;
    FT_Error error = FT_New_Memory_Face(library->library, reinterpret_cast<const FT_Byte *>(memory), static_cast<FT_Long>(memorySize), 0, &handle->face);
    if (error) {
        delete handle;
        return NULL;
    }
